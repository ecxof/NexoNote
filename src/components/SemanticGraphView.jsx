...
export default function SemanticGraphView({ note, notes = [], onClose, onOpenInTab }) {
  ...
  useEffect(() => {
    if (!note?.id) { setLoading(false); return; }
  ...
  const drawNode = useCallback((node, ctx, globalScale) => {
    ...
    if (node.isCenter || isSelected) {
      ctx.save();
      ctx.shadowColor = node.isCenter ? 'rgba(59,130,246,0.55)' : 'rgba(139,92,246,0.55)';
      ctx.shadowBlur = 14 / globalScale;
    }

    // Background fill.
    ctx.fillStyle = node.isCenter ? '#1e3a5f' : isSelected ? '#2d1b4e' : '#1e293b';
    ...
    // Border.
    ctx.strokeStyle = node.isCenter ? '#3b82f6' : isSelected ? '#8b5cf6' : '#334155';
  ...
  return (
    <div className="sgv-root">
      ...
      <header className="sgv-topbar">
        ...
        <button type="button" className="sgv-close-btn" onClick={onClose} aria-label="Close graph view">
          <X size={16} />
          Close View
        </button>
      </header>
      ...
      <div className="sgv-body">
        ...
        <div ref={containerRef} className="sgv-canvas-wrap">
          {loading ? (
            ...
            <div className="sgv-state-msg sgv-state-error">
              {fetchError}
            </div>
          ) : graphData.nodes.length <= 1 ? (
            <div className="sgv-state-msg">
              No related notes found yet. Add more notes with shared domain concepts (e.g. "backpropagation", "deadlock") and reopen this view.
            </div>
          ) : (
            <ForceGraph2D
              ref={graphRef}
        ...
        {/* ── Details panel ─────────────────────────────────────────────── */}
        <aside className={`sgv-panel${panelOpen ? ' sgv-panel-open' : ''}`} aria-label="Note details">
          {selectedNode && (
            <>
              {/* Header */}
              <div className="sgv-panel-header">
                ...
                <button type="button" className="sgv-panel-close" onClick={closePanel} aria-label="Close panel">
                  <X size={15} />
                </button>
              </div>
              ...
              <div className="sgv-panel-body">
                ...
                {!selectedNode.isCenter && (
                  <div className="sgv-panel-actions">
                    ...
                    <button type="button" className="sgv-btn-secondary" disabled title="Flashcards coming soon">
                ...
                {!selectedNode.isCenter && selectedNode.score > 0 && (
                  <div className="sgv-panel-score-wrap">
                    ...
                    <span className="sgv-panel-score-value">{Math.round(selectedNode.score * 100)}%</span>
                  </div>
                )}
                ...
                {connectedLink?.matched_keywords?.length > 0 && (
                  <section className="sgv-panel-concepts">
                    <h3 className="sgv-panel-concepts-title">Connected Concepts</h3>
                      ...
                      {connectedLink.matched_keywords.map((kw, i) => (
                        <li key={kw} className="sgv-panel-concept-item">
                          ...
                          <div className="sgv-panel-concept-text">
                            ...
                            <span className="sgv-panel-concept-sub">shared term</span>
                          </div>
                        </li>
                      ))}
                    </ul>
